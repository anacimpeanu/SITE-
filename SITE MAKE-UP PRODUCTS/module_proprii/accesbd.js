...
//implementeaza un singleton => crearea unei instante a unei clase
class AccesBD{

    //atributele de initializare , pentru aceeasi zona de memorie ale clasei 
    //ce e apelat cu numele static se foloseste cu numele clasei 

    static #instanta=null;
    static #initializat=false;

    //protejam constructorul deaorece este public
    constructor() {

        //daca avem instanta 
        if(AccesBD.#instanta){
            throw new Error("Deja a fost instantiat"); //vrem sa oprim crearea 
        }

        //daca nu e initializat 
        else if(!AccesBD.#initializat){
            throw new Error("Trebuie apelat doar din getInstanta; fara sa fi aruncat vreo eroare");
        }
        //vrem sa ne asiguram ca nu avem o noua instanta in afara clasei 
    }

    initLocal(){

        this.client= new Client({database:"kyracosmetics",
            user:"ana", 
            password:"1234", 
            host:"localhost", 
            port:5432});
    ...
    //returnez clientul pentru baza de date 
    getClient(){
        if(!AccesBD.#instanta ){
    ...
    //JSON imi permite niste hint-uri  
    /**
    ...
    static getInstanta({init="local"}={}){
       ...
       //creez o instanta 
        if(!this.#instanta){
            ...
            try{
                switch(init){
                    case "local":this.#instanta.initLocal(); //asta e pt baza de date locala 
                    //cazuri pentru a pune site-ul pe internet 
                }
    ...
    //obiect cu proprietatile cand apelez select ( select * from etc )
    select({tabel="",campuri=[],conditiiAnd=[]} = {}, callback, parametriQuery=[]){
        let conditieWhere="";
        //daca nu vid 
        if(conditiiAnd.length>0)
        //pune and intre fiecare
            conditieWhere=`where ${conditiiAnd.join(" and ")}`; //pune
        //o comanda simpla de select 
        //TO DO PENTRU OR //deci vreau in cate doua de and sa pun or 
    ...
    insert({tabel="",campuri={}} = {}, callback){
        console.log("-------------------------------------------")
        console.log(Object.keys(campuri).join(",")); //numele prorpeitatilor 
        //object -> mostenita de toate celelalte clase 
        console.log(Object.values(campuri).join(","));
        let comanda=`insert into ${tabel}(${Object.keys(campuri).join(",")}) values ( ${Object.values(campuri).map((x) => `'${x}'`).join(",")})`; //object.values vectori cu valori .map ia fiecare element din vector cu ce returneaza acea functie (=>)
        console.log(comanda);
...
//exporta clasa ca sa pot sa fac require 